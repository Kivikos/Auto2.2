...
public class CardDeliveryTest {
    ...
    String getFormattedDate(int days, String patternOfDate) {
        return LocalDate.now().plusDays(days).format(DateTimeFormatter.ofPattern(patternOfDate));
    }
    ...
    void shouldSendFormAnotherVar() {
        ...
        $("[data-test-id=agreement]>.checkbox__box").click();
        $("button>.button__content").click();
        $("[data-test-id=notification]").shouldBe(visible, Duration.ofSeconds(15)).shouldHave(exactText("Успешно! Встреча успешно забронирована на " + getFormattedDate(days, "dd.MM.yyyy")));
    }
}
